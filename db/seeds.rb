...
# Elective options per slot (3 credits each). Lifted from Neha's reference.
ELECTIVE_OPTIONS = {
  "Program Elective I" => [
    "Sensors and Actuators",
    "Biomedical Signal Processing and Transducers",
  ...
  "Open Elective" => [
    ...
    "Object-Oriented Programming (open basket)",
...
puts "Seeded #{Subject.count} subjects " \
     "(#{Subject.where(kind: 'elective_slot').count} slots, " \
     "#{Subject.where(kind: 'elective_option').count} options, " \
     "#{Subject.where(kind: 'vac').count} VAC entries)."

# Wipe any saved plan selections that referenced now-deleted elective options.
PlanSelection.where.not(subject_id: Subject.pluck(:id)).destroy_all
